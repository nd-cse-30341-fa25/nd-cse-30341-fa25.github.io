...
test_program() {
    ...
    elif [ "$NODIFF" = "" ] && ! diff -W 120 -y <(sort $WORKSPACE/test.stdout) <(find $ARGUMENTS 2> /dev/null | xargs $SHASUM $ARGUMENTS 2> /dev/null | sort | uniq) &> $WORKSPACE/test.diff; then
...
test_program "." 0
test_program ".." 0
test_program "Makefile . asdf /bin/ls" 1
test_program ". .. /root" 1
